...
const cursorPosInImage = (): Konva.Vector2d => {
  ...
  const fixedLayerPos = {
    x: stage.position().x / stage.scale().x + layer.position().x / layer.scale().x,
    y: stage.position().y / stage.scale().y + layer.position().y / layer.scale().y,
  }
...
const scalableStageConfig: ScalableStageConfig = calcStageConfig(width, height, image);
...
const childs: LayerChildType[] = [
  (() => {
    ...
    kImage.on("click", () => {
      ...
      console.debug(`onImageClick@ImageCanvas <cursorPos=${JSON.stringify(cursorPos)}>`);
  ...
  ...points.map((point, i) => {
    return new Point({
      ...
      onPointDrag: (index, pos) => {
        console.debug(
          `onPointDrag@ImageCanvas <index=${index}, pos=${JSON.stringify(pos)}>`
        )
